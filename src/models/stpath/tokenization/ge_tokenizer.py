...
class GeneExpTokenizer(TokenizerBase):
    ...
    def encode(self, adata, return_sparse=False):
        ...
        if not isinstance(adata.X, np.ndarray):
            ...
            row = np.repeat(np.arange(adata.X.shape[0]), counts_per_row)
            col = adata.X.indices
            act_gene_exp = adata.X.data
        else:
            row, col = np.nonzero(adata.X)
            act_gene_exp = adata.X[row, col]

        obs_gene_ids = np.array([self.gene2id[self.symbol2gene[g]] for g in adata.var_names.tolist()])
        ...
        if return_sparse:
            if coalesce is None:
                raise ImportError("torch_geometric is required for sparse encoding. Install via: pip install torch_geometric==2.6.1")
