...
class TransformerModel(nn.Module):
    def __init__(
        ...
        if use_generative_training:
            if FlashscGPTLayer is None:
                raise ImportError(
                    "flash_attn is required for generative training mode. "
                    "Install it or set use_generative_training=False."
                )
            ...
            if fast_transformer_backend == "linear":
                ...
                if not _FLASH_ATTN_AVAILABLE:
                    raise ImportError(
                        "flash_attn is required for fast_transformer with flash backend. "
                        "Install it or set use_fast_transformer=False."
                    )
    ...
    def _extend_output(
        ...
        if MVC_impute:
            ...
            NN_cells = transformer_output[topk_index, 0, :]
...
if _FLASH_ATTN_AVAILABLE:
    class FlashTransformerEncoderLayer(nn.Module):
        r"""TransformerEncoderLayer with FlashAttention support."""
    ...
    class FlashTransformerEncoderLayer:
        """Stub: flash_attn not available."""
        def __init__(self, *args, **kwargs):
            raise ImportError(
                "flash_attn is required for FlashTransformerEncoderLayer. "
                "Install it or use use_fast_transformer=False."
            )
