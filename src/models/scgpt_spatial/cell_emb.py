...
def get_batch_cell_embeddings(
    ...
    elif not isinstance(count_matrix, np.ndarray):
    ...
    class Dataset(torch.utils.data.Dataset):
        def __init__(self, count_matrix, gene_ids, gene_stats_dict_file, batch_ids=None):
            ...
            for i in new_genes:
                ...
                nonzero = col[col != 0]
                if nonzero.size == 0:
                    # Gene has no expression on this slide; __getitem__ only
                    # looks up per-row nonzero genes, so this row would never
                    # be read. Skip rather than store NaN.
                    continue
                self.gene_stats_dict.loc[i, 'mean'] = float(nonzero.mean())
...
def embed_data(
    ...
    # Copy to avoid ImplicitModificationWarning on views from adata slicing
    if adata.is_view:
        adata = adata.copy()
    adata.obsm["X_scgpt_spatial"] = cell_embeddings
    return adata
